...
| 2026-09-18 | **First live run against a real database.** Migration applied (20 `pricing_*` tables), module seeded into the existing tenant via a new `pricing_engine seed` CLI command (setup hooks only fire at tenant creation, so a module enabled later has no parameters at all). Verified end to end: `GET /coverage` 200 listing all 11 components with 9 implemented; `POST /quote` 200 with both ledger invariants holding on live data (`unitPrice x quantity === totalPriceNet`, waterfall summing to the price); `POST /simulate` 200 writing no ledger row; `GET /calculations/:id` 200 replaying 9 breakdown rows; 404 on an unknown id; 401 unauthenticated; both admin screens 200; tenant/organization scoping present on calculations and ledger lines. Two defects only a live run could surface: the ledger's `calculationId` was `undefined` because Postgres generates the PK (`gen_random_uuid()`) and MikroORM leaves `id` unset until flush — the id is now assigned in JS so parent and children persist in one flush, guarded by a recording-EntityManager regression test that was mutation-checked; and `URUCHOMIENIE.md` documented a JSON login body when `POST /api/auth/login` takes form-encoded data (a JSON body parses to empty credentials and returns a generic invalid-credentials 400). 97 unit tests. |
| 2026-09-18 | Step 2 implemented via a 5-lane parallel workflow with adversarial verification: `packaging_cost`, `warehouse_cost`, `logistics_cost`, `product_aspects` — 9 of 11 components now run. Verifiers reported 34 defects; the high-severity ones were fixed before merge. Notable corrections: frozen capital now finances the **purchase cost** rather than the accumulated cost (spec formula, was a real deviation); `ComponentComputeArgs.componentValues` added so a component can read a specific earlier component instead of the running total; `floorToInteger` added so carton counting never leaves the decimal domain; `product_aspects` multipliers clamped to (0, 5] so a configured `0` can no longer zero a price; logistics divisors validated against `0`/negative/non-numeric config; `product_aspects` confidence no longer treats a partial payload as configured. Added an i18n key-coverage guard (8 tests) that fails when any emitted explain/warning key is missing from a locale or has an unsupplied placeholder. Fixed a real DI defect caught by `core`'s `di-classic-proxy` test: `pricingService` needed `.proxy()` under Awilix CLASSIC. 94 unit tests. |
...
| 2026-09-19 | Framework-alignment cleanup, behaviour unchanged. `events.ts` now declares only events with a real emit site: `pricing_engine.calculation.created` is emitted from `pricingService.quote()` after the ledger flush, and `pricing_engine.parameters.changed` from every parameter CRUD route through the shared `pricingParameterEventHooks(entity)` (`lib/crud/pricingCrudRoute.ts`, `afterCreate`/`afterUpdate`/`afterDelete`). `mode.changed` and `supplier.imported` were removed — shadow mode is an env flag and no supplier import surface exists, so neither could ever fire; the matching ACL ids stay declared for the planned surfaces. |
| 2026-09-19 | **Document purchase cost on a quote line.** `quoteLineSchema` / `PricingBasketLine` gain optional `purchaseUnitCostNet`; when present, `product_cost` uses it verbatim (confidence `measured`, explain key `productCost.explain.document`, no stale/tier logic, no purchase position required). Lines without it are costed exactly as before. Motivated by the WZ comparison: re-pricing a historical document must show the goods cost the source booked, not a later delivery's replacement cost. Two component tests added. |

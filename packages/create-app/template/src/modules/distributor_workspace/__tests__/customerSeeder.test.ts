...
import { HORECA_CUSTOMERS, isRepeatOrderCustomer } from '../seed/horecaCustomerData'
...
type RecordingOptions = {
  /** Rows `em.find` hands back, by entity class. Defaults to none. */
  rows?: Map<unknown, Array<Record<string, unknown>>>
  /** Whether the `repeat_order` scenario probe resolves. Defaults to true. */
  repeatOrderScenario?: boolean
}

function recordingEm(
  options: RecordingOptions = {},
): { em: EntityManager; created: Created[]; persisted: Created[]; flushes: number } {
  const created: Created[] = []
  const persisted: Created[] = []
  let flushes = 0
  const em = {
    async findOne(type: unknown) {
      if (type === PricingOrderScenario) {
        return options.repeatOrderScenario === false ? null : { code: REPEAT_ORDER_SCENARIO_CODE }
      }
      return null
    },
    async find(type: unknown) {
      return options.rows?.get(type) ?? []
    },
...
describe('seedHorecaCustomers', () => {
  ...
  it('marks pricing profiles as demo data and carries the zone and scenario', async () => {
    ...
    for (const [index, profile] of profiles.entries()) {
      ...
      // Either the channel from the seed data, or `repeat_order` where the customer qualifies —
      // the rule itself is pinned in repeatOrderAssignment.test.ts.
      expect(profile.data.defaultOrderScenarioCode).toBe(
        isRepeatOrderCustomer(seed) ? REPEAT_ORDER_SCENARIO_CODE : seed.orderScenarioCode,
      )
